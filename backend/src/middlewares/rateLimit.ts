...
export function createRateLimit(options: RateLimitOptions) {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    if (entry.count >= options.max) {
      ...
      logger.warn(
        `Rate limit exceeded for IP ${clientIP} on ${req.path}. Reset in ${timeUntilReset}s`
      );
...
export const smartAISearchRateLimit = createSmartRateLimit({
  ...
  authenticatedMessage:
    'AI search rate limit exceeded for your account (10 requests per hour). Please try again later.',
});
...
export const aiSearchRateLimit = createRateLimit({
  ...
  message:
    'Daily AI search limit reached (3 requests per day). Please register for unlimited access or try again tomorrow.',
});
...
export function createSmartRateLimit(options: SmartRateLimitOptions) {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    if (entry.count >= currentMax) {
      ...
      logger.warn(
        `Rate limit exceeded for ${identifier} on ${req.path}. Reset in ${timeUntilReset}s`
      );
