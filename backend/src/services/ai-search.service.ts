 ...
 */
function deduplicateAndFilterResults(
  results: SearchResult[],
  minRelevanceScore: number = 1.0
): SearchResult[] {
...
export async function aiSearch(query: string): Promise<SearchResult[]> {
  try {
    ...
    users.forEach(user => {
      ...
      if (
        processedQuery.intent.toLowerCase().includes('people') ||
      ...
      if (score > 0) {
        results.push({
          ...
          description:
            user.bio ||
            `${user.firstName} ${user.lastName} - ${user.role} at ${user.organization || 'Unknown'}`,
          type: 'user',
    ...
    challenges.forEach(challenge => {
      ...
      // Add intent-based scoring
      if (
        processedQuery.intent.toLowerCase().includes('challenge') ||
    ...
    partnerships.forEach((partnership: any) => {
      ...
      // Add intent-based scoring
      if (
        processedQuery.intent.toLowerCase().includes('partnership') ||
    ...
    ideas.forEach(idea => {
      ...
      // Add intent-based scoring
      if (
        processedQuery.intent.toLowerCase().includes('idea') ||
      ...
      if (
        processedQuery.filters.category &&
