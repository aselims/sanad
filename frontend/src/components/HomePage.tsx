...
export function HomePage({ 
  ...
  console.log('HomePage initialSearchResults:', initialSearchResults);
  console.log('HomePage searchResults state:', searchResults);

  // Add a useEffect to update searchQuery and handle search results when props change
  useEffect(() => {
    ...
    if (initialSearchResults) {
      ...
      // Check if there are no results and set error message
      if (initialSearchResults.innovators.length === 0 && 
          initialSearchResults.collaborations.length === 0 && 
          (!initialSearchResults.aiResults || initialSearchResults.aiResults.length === 0) &&
          initialSearchQuery) {
        setSearchError(`No results found for "${initialSearchQuery}"`);
      } else {
        setSearchError(null);
      }
    }
  }, [initialSearchResults, initialSearchQuery]);

  // Add debugging logs after the useEffect to track when component receives new props
  console.log('HomePage rendered with props:', {
    initialSearchQuery,
    hasResults: initialSearchResults && (
      initialSearchResults.innovators.length > 0 || 
      initialSearchResults.collaborations.length > 0 || 
      (initialSearchResults.aiResults && initialSearchResults.aiResults.length > 0)
    )
  });
  ...
  // Debug log for search results presence
  console.log('Search results check:', {
    hasSearchResults,
    innovatorsLength: searchResults.innovators.length,
    collaborationsLength: searchResults.collaborations.length,
    aiResultsLength: searchResults.aiResults?.length || 0
  });
  
  // Debug search results rendering with useEffect
  useEffect(() => {
    console.log('About to render search results section, condition:', (hasSearchResults || searchError));
    if (hasSearchResults || searchError) {
      console.log('Rendering search results section would happen now');
    }
